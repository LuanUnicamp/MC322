...
public class App {
    ...
    /**
     * Renderiza a tela de encerramento do jogo com artes ASCII temáticas.
     * <br>
     * <b>Comportamento:</b>
     * <ul>
     * <li>Limpa o terminal para focar na ilustração.</li>
     * <li>Se {@code venceu} for verdadeiro, exibe o herói em tons de verde e amarelo.</li>
     * <li>Se {@code venceu} for falso, exibe o vilão em tons de vermelho e roxo.</li>
     * </ul>
     * * @param venceu inteiro que indica o desfecho do combate. 
     * {@code true} para vitória do jogador, {@code false} para derrota.
     */
    public static void exibirFinal(int venceu) {
...
⠿⠛⠋⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⣫⡆⣼⣷⣌⠡⢀⡄⣡⠊⠔⠔⢁⣾⣿⣿⣿⠇⢀⡦⠀⠀⠀⠀⠀⠀⠀⠀⠀⢠⠂⠀⠀⠀⠈⠛⠿⣿⣿⣿⣿""");    
    ...
    /**
    * Gera uma representação visual da barra de vida.
    * @param atual Vida atual
    * @param max Vida máxima (ex: 100)
    * @param tamanho Quantidade de caracteres da barra
    * @param cor Cor ANSI da barra
    * @return String formatada [■■■■□□□□]
    */
    public static String gerarBarraVida(int atual, int max, int tamanho, String cor) {
        if (atual < 0) atual = 0;
        int preenchido = (int) ((double) atual / max * tamanho);
        if (preenchido > tamanho) preenchido = tamanho;
        int vazio = tamanho - preenchido;

        String barra = cor + "▰".repeat(preenchido) + RESET + "▱".repeat(vazio);
        return "[" + barra + "] " + AMARELO + atual + "/" + max + RESET;
    }
    ...
    public static void display(Heroi h, Inimigo i, int chakra, ArrayList<Carta> maoJogador, ArrayList<Carta> movimentosInimigo){
        ...
        String barraH = gerarBarraVida(h.getVida(), h.getVidaMax(), 12, VERDE);
        ...
        System.out.printf("  Escudo: %-30s      Escudo: %s\n", escH, escI);
